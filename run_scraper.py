...
if countries_str == 'ALL':name: Daily Racing Form Update (Enhanced)
    ...
    steps:
      ...
      - name: Run racing scraper
        env:
          ...
          PYTHONPATH: ${{ github.workspace }}/Ladbrokes
        run: |
          python Ladbrokes/run_scraper.py
        continue-on-error: false

      - name: Verify data was scraped
        run: |
          if [ -d "racing_data" ] && [ "$(ls -A racing_data)" ]; then
            echo "✓ Racing data successfully scraped"
      ...
      - name: Commit and push changes
        run: |
          ...
          if git diff --staged --quiet; then
            ...
            git commit -m "🏇 Racing form update - ${{ steps.config.outputs.timestamp }} [${{ steps.config.outputs.countries }}]"
      ...
      - name: Generate summary
        run: |
          echo "## 🏇 Racing Form Update Complete" >> $GITHUB_STEP_SUMMARY
      ...
      - name: Notify on failure
        if: failure()
        run: |
          echo "❌ Scraping failed - check logs above" >> $GITHUB_STEP_SUMMARY
